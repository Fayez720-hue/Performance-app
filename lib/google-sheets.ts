...
async function getLastNonEmptyRow(sheetName: string, column: string = 'A'): Promise<number> {
  try {
    // Standard Google Sheets API range format
    const range = `'${sheetName.replace(/'/g, "''")}'!${column}:${column}`;
    // We only encode the whole range if it contains spaces, but keep ! and : as they are delimiters
    const path = `/values/${range}`;
    const data = await sheetsRequest(path);
    const rows = data.values || [];

    // Scan backwards to find the last truly non-empty row
...
async function sheetsRequest(path: string, options: RequestInit = {}) {
  ...
  // Use a cleaner URL construction without double-encoding delimiters
  ...
  if (!response.ok) {
    let errorDetail = "";
    try { errorDetail = JSON.stringify(await response.json()); } catch { errorDetail = response.statusText; }
    console.error(`Sheets API Error (${response.status}):`, errorDetail)
...
export async function addUser(data: { email: string; name: string; role: UserRole; title?: string }): Promise<void> {
  ...
  await sheetsRequest(`/values/Employees!A${nextRow}:J${nextRow}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
...
export async function createTask(data: any): Promise<number> {
  ...
  // Use literal range to avoid delimiter encoding issues
  const mainRange = `Performance!A${nextRow}:T${nextRow}`;
  await sheetsRequest(`/values/${mainRange}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
    body: JSON.stringify({ values })
  })
  ...
  if (userSheetName) {
    try {
      ...
      const userRange = `'${userSheetName.replace(/'/g, "''")}'!A${userNextRow}:S${userNextRow}`;
      await sheetsRequest(`/values/${userRange}?valueInputOption=USER_ENTERED`, {
        method: "PUT",
        body: JSON.stringify({ values })
      });
    } catch (err) {
      console.log(`User sheet ${userSheetName} sync failed, skipping.`);
    }
  }
...
export async function updateTask(id: number, data: any): Promise<void> {
  ...
  if (rowIndex === -1) throw new Error(`Task ID ${id} not found`)

  const realRowNumber = rowIndex + 1
  const values = [[
    id, data.name || "", data.date || "", data.task || "", data.references || "",
    data.comments || "", data.progress || "To-do", data.taskStartingDate || "",
    data.deadline || "", data.taskEstimatedTime || "", data.taskTimeTaken || "",
    data.submissionLink || "", data.submissionDate || "", data.deadlineAdherence || "",
    data.grading || "", data.overallScore || "", data.taskTimeStamp || "",
    data.edits || "", data.noOfEdits || 0, data.performanceHistory || ""
  ]]
...
export async function getDashboardStats(startDate?: string, endDate?: string, userEmail?: string, userRole?: string) {
  try {
    const [summaryData, employeeData, tasksData] = await Promise.all([
      sheetsRequest("Summary!A2:I2"),
      sheetsRequest("Employees!A2:I"),
      sheetsRequest("Performance!A2:T")
    ])
    ...
    if (!isAdminOrManager && userEmail) {
      ...
      relevantTasks = taskRows.filter((row: any[]) => {
        ...
        return taskName === currentEmployee?.name.toLowerCase() || taskName === userEmail.toLowerCase()
      })
      totalTasks = relevantTasks.length
      completedTasks = relevantTasks.filter((row: any[]) => String(row[6] || "").trim().toLowerCase() === "completed").length
      employeeStats = currentEmployee ? [currentEmployee] : []
    } else {
    ...
    relevantTasks.forEach(row => {
      ...
      if (adhStr.includes("%")) {
        totalAdherence += parseFloat(adhStr)
        adherenceCount++
      }
    ...
    return {
      ...
      totalTasks, completedTasks, avgShiftAdherence: avgAdherence,
...
export async function savePushToken(email: string, token: string): Promise<void> {
  ...
  await sheetsRequest(`Employees!${colLetter}${realRowNumber}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
...
export async function clockIn(email: string, name: string): Promise<void> {
  ...
  await sheetsRequest(`Attendance!A${nextRow}:H${nextRow}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
    body: JSON.stringify({ values })
  })
}
...
export async function clockOut(email: string): Promise<void> {
  ...
  if (rowIndex === -1) throw new Error("No active clock-in found")
  ...
  if (row[5]) {
    try {
      ...
      row[2] = `${hours.toFixed(2)} hours`
    } catch { row[2] = "Error calc" }
  }

  await sheetsRequest(`Attendance!A${realRowNumber}:H${realRowNumber}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
...
export async function createNotification(notification: Notification): Promise<void> {
  const values = [[
    notification.userEmail, notification.type, notification.taskId || "",
    notification.message, notification.read ? "TRUE" : "FALSE", notification.timestamp,
  ]]

  const lastRow = await getLastNonEmptyRow('Notifications', 'A');
  const nextRow = lastRow + 1;

  await sheetsRequest(`Notifications!A${nextRow}:F${nextRow}?valueInputOption=USER_ENTERED`, {
    method: "PUT",
    body: JSON.stringify({ values }),
  })
}
...
export async function getNotifications(email: string): Promise<Notification[]> {
  try {
    ...
    return rows
      ...
      .map((row: any[], index: number) => ({
        id: `notification-${index}-${row[5]}`,
        userEmail: row[0], type: row[1] as any, taskId: parseInt(row[2]) || 0,
        message: row[3], read: row[4] === "TRUE", timestamp: row[5],
      }))
...
export async function markNotificationAsRead(email: string, timestamp: string): Promise<void> {
  ...
  if (rowIndex !== -1) {
    ...
    const row = rows[rowIndex]
    row[4] = "TRUE"
    await sheetsRequest(`Notifications!A${realRowNumber}:F${realRowNumber}?valueInputOption=USER_ENTERED`, {
      method: "PUT",
      body: JSON.stringify({ values: [row] }),
    })
  }
